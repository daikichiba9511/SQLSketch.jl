...
@testset "ON CONFLICT - Edge Cases" begin
    ...
    upsert_func = base_insert |>
        on_conflict_do_update([:email], :updated_at => func(:CURRENT_TIMESTAMP, SQLExpr[]))
...
@testset "SQLite - ON CONFLICT DO NOTHING Compilation" begin
    ...
    @test sql2 == "INSERT INTO `users` (`id`, `email`) VALUES (?, ?) ON CONFLICT (`email`) DO NOTHING"
    ...
    @test sql3 == "INSERT INTO `users` (`id`, `email`, `name`) VALUES (?, ?, ?) ON CONFLICT (`email`, `name`) DO NOTHING"
...
@testset "SQLite - ON CONFLICT DO UPDATE Compilation" begin
    ...
    sql, params = compile(dialect, q)
    @test sql == "INSERT INTO `users` (`id`, `email`, `name`) VALUES (?, ?, ?) ON CONFLICT (`email`) DO UPDATE SET `name` = `excluded`.`name`"
    @test params == [:id, :email, :name]

    # Multiple update columns
    q2 = insert_into(:users,[:id, :email, :name, :active]) |>
        values([[param(Int, :id), param(String, :email), param(String, :name), param(Bool, :active)]]) |>
        on_conflict_do_update(
            [:email],
            :name => col(:excluded, :name),
    ...
    sql2, params2 = compile(dialect, q2)
    @test sql2 == "INSERT INTO `users` (`id`, `email`, `name`, `active`) VALUES (?, ?, ?, ?) ON CONFLICT (`email`) DO UPDATE SET `name` = `excluded`.`name`, `active` = `excluded`.`active`"
    ...
    sql3, params3 = compile(dialect, q3)
    @test sql3 == "INSERT INTO `users` (`id`, `email`) VALUES (?, ?) ON CONFLICT (`email`) DO UPDATE SET `active` = 1"
    ...
    @test sql4 == "INSERT INTO `users` (`id`, `email`) VALUES (?, ?) ON CONFLICT (`email`) DO UPDATE SET `updated_at` = CURRENT_TIMESTAMP()"
    ...
    @test sql5 == "INSERT INTO `users` (`id`, `email`, `login_count`) VALUES (?, ?, ?) ON CONFLICT (`email`) DO UPDATE SET `login_count` = (`users`.`login_count` + 1)"
...
@testset "SQLite - ON CONFLICT with WHERE Clause" begin
    ...
    q = insert_into(:users,[:id, :email, :name, :version]) |>
        values([[param(Int, :id), param(String, :email), param(String, :name), param(Int, :version)]]) |>
        on_conflict_do_update(
            [:email],
            :name => col(:excluded, :name);
            where = col(:users, :version) < col(:excluded, :version)
    ...
    sql, params = compile(dialect, q)
    @test sql == "INSERT INTO `users` (`id`, `email`, `name`, `version`) VALUES (?, ?, ?, ?) ON CONFLICT (`email`) DO UPDATE SET `name` = `excluded`.`name` WHERE (`users`.`version` < `excluded`.`version`)"
    ...
    sql2, params2 = compile(dialect, q2)
    @test sql2 == "INSERT INTO `users` (`id`, `email`, `active`) VALUES (?, ?, ?) ON CONFLICT (`email`) DO UPDATE SET `active` = `excluded`.`active` WHERE (`users`.`active` = 0)"
...
@testset "SQLite - ON CONFLICT Edge Cases" begin
    ...
    sql3, params3 = compile(dialect, q3)
    @test sql3 == "INSERT INTO `users` (`id`, `email`) VALUES (?, ?) ON CONFLICT (`id`) DO NOTHING"
end
...
@testset "SQLite - ON CONFLICT with Parameters" begin
    ...
    q = insert_into(:users,[:id, :email, :name]) |>
        values([[param(Int, :user_id), param(String, :user_email), param(String, :user_name)]]) |>
        on_conflict_do_update(
            [:email],
    ...
    sql, params = compile(dialect, q)
    @test sql == "INSERT INTO `users` (`id`, `email`, `name`) VALUES (?, ?, ?) ON CONFLICT (`email`) DO UPDATE SET `name` = ?"
    ...
    sql2, params2 = compile(dialect, q2)
    @test sql2 == "INSERT INTO `users` (`id`, `email`, `active`) VALUES (?, ?, 1) ON CONFLICT (`email`) DO UPDATE SET `active` = 1, `updated_at` = CURRENT_TIMESTAMP()"
