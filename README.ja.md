...
### クエリ構築
| 機能 | SQLite | PostgreSQL | MySQL | 説明 |
|------|--------|------------|-------|------|
| **SELECT** | ✅ | ✅ | ✅ | 基本・複雑な射影 |
| **WHERE** | ✅ | ✅ | ✅ | 式によるフィルタリング |
| **JOIN** | ✅ | ✅ | ✅ | INNER, LEFT, RIGHT, FULL |
| **GROUP BY** | ✅ | ✅ | ✅ | 集約のためのグループ化 |
| **HAVING** | ✅ | ✅ | ✅ | 集約後のフィルタリング |
| **ORDER BY** | ✅ | ✅ | ✅ | ASC/DESCソート |
| **LIMIT/OFFSET** | ✅ | ✅ | ✅ | 結果のページネーション |
| **DISTINCT** | ✅ | ✅ | ✅ | 重複行の削除 |
| **サブクエリ** | ✅ | ✅ | ✅ | ネストされたSELECT式 |
| **CTE (WITH)** | ✅ | ✅ | ✅ | 共通テーブル式 |
...
| **CASE式** | ✅ | ✅ | ✅ | 条件分岐（CASE WHEN） |
| **CAST** | ✅ | ✅ | ✅ | 型変換 |
| **COALESCE** | ✅ | ✅ | ✅ | NULL処理（func()経由） |
| **LIKE/ILIKE** | ✅ | ✅ | ✅ | パターンマッチング |
| **IN/NOT IN** | ✅ | ✅ | ✅ | リスト検索 |
| **BETWEEN** | ✅ | ✅ | ✅ | 範囲クエリ |
...
| **比較演算子** | ✅ | ✅ | ✅ | =, !=, <, >, <=, >= |
| **論理演算子** | ✅ | ✅ | ✅ | AND, OR, NOT |
| **文字列関数** | ✅ | ✅ | ✅ | CONCAT, UPPER, LOWER等（func()経由） |
| **数値関数** | ✅ | ✅ | ✅ | ROUND, ABS, POW等（func()経由） |
| **日時関数** | ✅ | ✅ | ✅ | NOW, DATE等（func()経由） |
...
| **RETURNING** | ✅ | ✅ | ❌ | 変更された行を返す |
| **バッチINSERT** | ✅ | ✅ (COPY) | ✅ | 高性能な一括挿入 |

### 高度な機能
| 機能 | SQLite | PostgreSQL | MySQL | 説明 |
|------|--------|------------|-------|------|
| **ウィンドウ関数** | ✅ | ✅ | ✅ | OVER, PARTITION BY, ROW_NUMBER, RANK等 |
| **集合演算** | ✅ | ✅ | ✅ | UNION, INTERSECT, EXCEPT |
| **トランザクション** | ✅ | ✅ | ✅ | BEGIN, COMMIT, ROLLBACK |
| **セーブポイント** | ✅ | ✅ | ✅ | ネストされたトランザクション |
| **プリペアドステートメント** | ❌ | ✅ | ✅ | ステートメントキャッシング（10-20%高速化） |
| **コネクションプーリング** | ❌ | ✅ | ✅ | スレッドセーフな接続再利用 |

### DDL（スキーマ管理）
| 機能 | SQLite | PostgreSQL | MySQL | 説明 |
|------|--------|------------|-------|------|
| **CREATE TABLE** | ✅ | ✅ | ✅ | 制約付きテーブル作成 |
| **ALTER TABLE** | ✅ | ✅ | ✅ | カラム追加・削除 |
| **DROP TABLE** | ✅ | ✅ | ✅ | テーブル削除 |
| **CREATE INDEX** | ✅ | ✅ | ✅ | インデックス作成（UNIQUE、複数列） |
| **DROP INDEX** | ✅ | ✅ | ✅ | インデックス削除 |
| **PRIMARY KEY** | ✅ | ✅ | ✅ | カラム・テーブル制約 |
| **FOREIGN KEY** | ✅ | ✅ | ✅ | 参照整合性 |
| **UNIQUE** | ✅ | ✅ | ✅ | 一意性制約 |
| **NOT NULL** | ✅ | ✅ | ✅ | 非NULL制約 |
| **DEFAULT** | ✅ | ✅ | ✅ | デフォルト値 |
| **CHECK** | ✅ | ✅ | ✅ | カスタム検証 |
| **AUTO_INCREMENT** | ✅ | ✅ (SERIAL) | ✅ | 自動採番ID |
| **マイグレーション** | ✅ | ✅ | ✅ | バージョンベースのスキーママイグレーション |
| **CREATE VIEW** | ❌ | ❌ | ❌ | 未サポート |
| **トリガー** | ❌ | ❌ | ❌ | 未サポート |
| **ストアドプロシージャ** | ❌ | ❌ | ❌ | 未サポート |
...
| 機能 | SQLite | PostgreSQL | MySQL | 説明 |
|------|--------|------------|-------|------|
| **Integer, Float, String** | ✅ | ✅ | ✅ | 基本型 |
| **Boolean** | ✅ | ✅ | ✅ | ネイティブまたはエミュレート |
| **Date, DateTime** | ✅ | ✅ | ✅ | 時間型 |
| **UUID** | ✅ | ✅ (ネイティブ) | ✅ | 一意識別子 |
| **JSON** | ✅ | ✅ (JSONB) | ✅ | JSONドキュメント |
| **配列** | ❌ | ✅ (ネイティブ) | ❌ | ネイティブ配列サポート |
| **NULL/Missing** | ✅ | ✅ | ✅ | NULL処理 |

### パフォーマンス最適化
| 機能 | SQLite | PostgreSQL | MySQL | 説明 |
|------|--------|------------|-------|------|
| **クエリプランキャッシュ** | ✅ | ✅ | ✅ | 4.85-6.95倍のコンパイル高速化 |
| **プリペアドStmtキャッシュ** | ❌ | ✅ | ✅ | 10-20%の実行高速化 |
| **コネクションプール** | ❌ | ✅ | ✅ | 4-5倍高速化（並行処理） |
| **バッチINSERT** | ✅ | ✅ (COPY) | ✅ | 1.35倍〜2016倍高速化 |
| **カラムナーフェッチ** | ✅ | ✅ | ✅ | 分析で8-10倍高速 |
| **@timed_query** | ✅ | ✅ | ✅ | パフォーマンスプロファイリング |
| **EXPLAIN分析** | ✅ | ✅ | ✅ | クエリプラン検査 |
